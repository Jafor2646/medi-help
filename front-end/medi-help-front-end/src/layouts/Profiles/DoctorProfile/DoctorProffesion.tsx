...
import DoctorProfessionalQualificationModel from "../../../models/DoctorProfessionalQualificationModel";

export const DoctorProffesion= () => {
    const { isAuthorised, current_user_id } = useContext(UserContext);
    const [postOpen, setpostOpen] = useState("false");
    const {globalUserId} = useContext(GlobalContext);
    const [doctorEdList, setdoctorEdList] = useState<DoctorProfessionalQualificationModel[]>([]);
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            const url: string = `${baseUrl}/doctorProfessionalQualifications/search/findAllByDoctorUserId?doctorUserId=${globalUserId}`;
            console.log(url);
            ...
            let tempArr:DoctorProfessionalQualificationModel[] = [];
            for (const key in responseData){

                let temp_ed: DoctorProfessionalQualificationModel = new DoctorProfessionalQualificationModel(responseData[key].currentlyWorkingHospitalInfoId, responseData[key].doctorUserId, responseData[key].startingYear, responseData[key].endingYear);
